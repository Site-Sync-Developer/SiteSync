...
export function NewConversationScreen() {
  ...
  const canCurrentUserSeeTarget = (target: User) => {
    if (user?.role === 'superadmin') return target.role === 'admin';
    if (target.role === 'superadmin') return user?.role === 'admin';
    return true;
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.hint}>Choose someone to message. A direct chat will open.</Text>
      <FlatList
        data={others}
