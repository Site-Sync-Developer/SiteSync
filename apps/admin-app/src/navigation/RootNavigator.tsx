...
import { AdminRegisterInfoScreen } from '../screens/auth/AdminRegisterInfoScreen';
import { ChangePasswordScreen } from '../screens/auth/ChangePasswordScreen';
import { SupervisorProjectSelectScreen } from '../screens/auth/SupervisorProjectSelectScreen';
import type { AdminAuthStackParamList } from './authTypes';

const Stack = createNativeStackNavigator();
const Auth = createNativeStackNavigator<AdminAuthStackParamList>();
...
function AdminAuthStack() {
  return (
    <Auth.Navigator screenOptions={{ headerShown: false }}>
      ...
      <Auth.Screen name="AdminRegisterInfo" component={AdminRegisterInfoScreen} />
    </Auth.Navigator>
  );
}
...
export function RootNavigator() {
  ...
  if (isAuthenticated) {
    const mustPickProject = user?.role === 'supervisor' && (projectCheckLoading || requiresProjectPick || !activeProjectId);
    if (mustPickProject) {
      return (
        <Stack.Navigator screenOptions={{ headerShown: false }}>
  ...
  return (
    <Stack.Navigator screenOptions={{ headerShown: false }}>
      <Stack.Screen name="Auth" component={AdminAuthStack} />
    </Stack.Navigator>
  );
}
